...
# -*- coding: utf-8 -*-
"""
Train StarCoderBase (4-bit QLoRA via Unsloth) to map:
...
Setup:
  pip install -U unsloth bitsandbytes transformers accelerate datasets trl
"""
import os
import json
import random
...
# ---- Unsloth (replaces HF AutoModel + peft) ----
...
def mask_tokens_with_pad(
    ...
    - We insert the string `<pad>` in the text so the model learns to read sketches with holes.
    """
...
def main():
    ...
    ap.add_argument("--output_dir", default="chkpts/scb_filler_sketch_qlora_unsloth", help="Where to save adapters")
    ...
    ap.add_argument("--checkpoint", default="bigcode/starcoderbase-1b", help="HF model id (e.g., bigcode/starcoderbase-3b or -7b)")
    ...
    checkpoint = args.checkpoint  # e.g., "bigcode/starcoderbase-3b"

    # -------- Load model & tokenizer with Unsloth (4-bit) --------
    # Unsloth passes these through to bitsandbytes / Transformers under the hood.
    ...
    # Teach the literal "<pad>" as an extra special string (not the pad token for padding)
    ...
    # Enable Unsloth’s optimized training path (grad ckpt, fused kernels where available)
    # Apply LoRA directly with Unsloth (StarCoder uses GPT-NeoX/GPT-2 style blocks: c_attn, c_proj, c_fc)
    model = FastLanguageModel.get_peft_model(
        ...
        use_gradient_checkpointing="unsloth",   # lowest VRAM
    )

    # --- Load JSONL as tiny HF datasets and split ---
    def gen():
    ...
    training_args = TrainingArguments(
        ...
        save_total_limit=2,
        # bf16=True,
        gradient_checkpointing=True,   # Unsloth also handles this internally
        optim="paged_adamw_8bit",      # 8-bit optimizer to save VRAM
        report_to="none",
        seed=args.seed,
        tf32=True,                     # on Ampere+ for speed
    )
    ...
    trainer.train()
    # Save LoRA adapters (PEFT) + tokenizer
